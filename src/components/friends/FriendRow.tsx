...
export default function FriendRow({
  ...
  const handlePlay = () => {
    if (activeRoom) {
      ...
      router.push(`/create?invite=${encodeURIComponent(friend.userId)}`);
    }
  };
    ...
    <div className="flex items-center gap-3 rounded-2xl border border-line bg-surface p-3">
      ...
      <div className="flex shrink-0 items-center gap-1.5">
        {activeRoom && (joined || declined || invited) ? (
          ...
          <PillButton color={GREEN} className="shrink-0" onClick={handlePlay} icon={<IconPlay className="h-3 w-3" />}>
            Play
          </PillButton>
        ) : (
          <PillButton color={ACCENT} className="shrink-0" onClick={handlePlay} icon={<IconSend className="h-3.5 w-3.5" />}>
            Invite
          </PillButton>
        )}
        <button
